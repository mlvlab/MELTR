...
from agent import Agent_Base_MELTR
import random
...
class VIOLET_Retrieval(VIOLET_Base):
    ...
    def forward(self, img, txt, mask, m_txt):
        ...
        feat_img, mask_img, feat_txt, mask_txt, feat_m_txt, mask_m_txt = self.go_feat(img, txt, mask, m_txt)
        ...
        out_vtm = self.fc(out[:, (1+_h*_w)*_T, :]).squeeze().view([_B, _B]) / 0.05
        
        ans_vtm = T.tensor([i for i in range(_B)]).long().cuda()
        
        return out_mtm, out_mvm, out_vtm, ans_vtm

class Agent_Retrieval(Agent_Base_MELTR):
    ...
    def step(self, img, txt, mask, ans_mtm, ans_mvm, m_txt, pri=False):
        img, txt, mask, ans_mtm, ans_mvm, m_txt = [x.cuda() for x in [img, txt, mask, ans_mtm, ans_mvm, m_txt]]
        self.optzr.zero_grad()
        
        out_mtm, out_mvm, out_vtm, ans_vtm = self.model(img, txt, mask, m_txt)
        ...
        if pri:
            ...
            return [ls_vtm, ls_mtm, ls_mvm]
    
    def go_dl(self, dl, args, e, global_step):
    
        aux_loss_0, aux_loss_1, aux_loss_2, pri_loss = AverageMeter(), AverageMeter(), AverageMeter(), AverageMeter()
        batchs, m_ans_mtm = [], T.tensor([0])
        for i, (img, txt, mask, vq) in enumerate(tqdm(dl)):
            img, m_txt, ans_mtm, ans_mvm = self.masking(img, txt.detach().clone(), vq)

            batch = [img, txt, mask, ans_mtm, ans_mvm, m_txt]
            losses = self.step(img, txt, mask, ans_mtm, ans_mvm, m_txt)
            ...
            if (global_step) % args['auxgrad_every'] == 0:
                ...
                if len(batchs) > 0:
                    img, txt, mask, ans_mtm, ans_mvm, m_txt = random.choices(batchs)[0]

                losses_pri = self.step(img, txt, mask, ans_mtm, ans_mvm, m_txt, pri=True)
                ...
                losses_train = self.step(img, txt, mask, ans_mtm, ans_mvm, m_txt)
        ...
        return aux_loss_0, aux_loss_1, aux_loss_2, pri_loss
...
if __name__=='__main__':
    ...
    dl_tr, dl_vl = [T.utils.data.DataLoader(Dataset_Retrieval(args, split), 
                                            ...
                                            num_workers=32, pin_memory=True) for split in ['train', 'val']]
    
    print(len(dl_tr), len(dl_vl))
    ...
    auxnet_config = dict(t_dim=3, f_dim=512, i_dim=1, h1_dim=256, h2_dim=512, o_dim=1)
    aux_model = T.nn.DataParallel(MELTR(**auxnet_config).cuda())
    
    
    agent = Agent_Retrieval(args, model, aux_model)
    global_step = 1
    for e in tqdm(range(args['size_epoch']), ascii=True):
        model.train()
        aux_model.train()
        aux_loss_0, aux_loss_1, aux_loss_2, pri_loss = agent.go_dl(dl_tr, args, e, global_step)
        
        T.save(model.module.state_dict(), '%s/ckpt_violet_%s_%d.pt'%(args['path_output'], args['task'], e+1))
        print(f'Ep {e+1:d}: {aux_loss_0.avg} {aux_loss_1.avg} {aux_loss_2.avg} {pri_loss.avg}')
