...
class Dataset_QAOE(Dataset_Base):
    ...
    def __getitem__(self, idx):
        ...
        txt, mask = self.str2txt(self.data[idx]['question'])
...
class VIOLET_QAOE(VIOLET_Base):
    def __init__(self, size_vocab):
        ...
        self.fc_oe = T.nn.Sequential(*[T.nn.Dropout(0.5), 
    ...
    def forward(self, img, txt, mask, m_txt):
        ...
        out_oe = self.fc_oe(out[:, (1+_h*_w)*_T, :])
        ...
        return out_mtm, out_mvm, out_vtm, out_oe, ans_vtm   
    
class Agent_QAOE(Agent_Base_MELTR):
    ...
    def step(self, img, txt, mask, ans_oe, ans_mtm, ans_mvm, m_txt, pri=False):
        img, txt, mask, ans_oe, ans_mtm, ans_mvm, m_txt = [x.cuda() for x in [img, txt, mask, ans_oe, ans_mtm, ans_mvm, m_txt]]
        self.optzr.zero_grad()
        
        out_mtm, out_mvm, out_vtm, out_oe, ans_vtm = self.model(img, txt, mask, m_txt)
    
        ls_oe = self.loss_func(out_oe, ans_oe)
        ...
        if pri:
            ...
            return [ls_oe, ls_vtm, ls_mtm, ls_mvm]
    
    def test(self, dl):
        ret = []
        for img, txt, mask, vq, ans_oe in tqdm(dl, ascii=True):
            m_img, m_txt, ans_mtm, ans_mvm = self.masking(img.detach().clone(), txt.detach().clone(), vq)
            out_mtm, out_mvm, out_vtm, out_oe, ans_vtm = self.model(img, txt, mask, m_txt)
            out = T.argmax(out_oe, dim=1)
            ac = (out == ans_oe.cuda()).float().mean().item()
            ret.append(ac)
    ...
    def train(self, dl, args, e, global_step):
        ...
        for i, (img, txt, mask, vq, ans_oe) in enumerate(tqdm(dl)):
    
            img, m_txt, ans_mtm, ans_mvm = self.masking(img, txt.detach().clone(), vq)
            
            batch = [img, txt, mask, ans_oe, ans_mtm, ans_mvm, m_txt]

            losses = self.step(img, txt, mask, ans_oe, ans_mtm, ans_mvm, m_txt)
            ...
            if (global_step) % args['auxgrad_every'] == 0:
                ...
                if len(batchs) > 0:
                    img, txt, mask, ans_oe, ans_mtm, ans_mvm, m_txt = random.choices(batchs)[0]

                losses_pri = self.step(img, txt, mask, ans_oe, ans_mtm, ans_mvm, m_txt, pri=True)
                meta_val_loss = sum(losses_pri)

                if len(batchs) > 0:
                    img, txt, mask, ans_oe, ans_mtm, ans_mvm, m_txt = random.choices(batchs)[0]

                losses_train = self.step(img, txt, mask, ans_oe, ans_mtm, ans_mvm, m_txt)
...
if __name__=='__main__':
    ...
    dl_tr, dl_ts = [T.utils.data.DataLoader(Dataset_QAOE(args, split), 
    ...
    agent = Agent_QAOE(args, model, aux_model)
    global_step = 1
