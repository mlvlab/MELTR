...
class Youtube_DataLoader(Dataset):
    ...
    def _get_text(self, video_id, n_pair_max, sub_ids=None, only_sim=False, enhance_vmodel=False):
        data_dict = self.data_dict[video_id] # ['text', 'start', 'end']
        ...
        starts = np.zeros(k) # (1, 32)
        ends = np.zeros(k) # (1, 32)
        pairs_text = np.zeros((k, self.max_words), dtype=np.long) # (1, 32)
        pairs_mask = np.zeros((k, self.max_words), dtype=np.long) # (1, 32)
        pairs_segment = np.zeros((k, self.max_words), dtype=np.long) # (1, 32)
        pairs_masked_text = np.zeros((k, self.max_words), dtype=np.long) # (1, 32)
        pairs_token_labels = np.zeros((k, self.max_words), dtype=np.long) # (1, 32)

        pairs_input_caption_ids = np.zeros((k, self.max_words), dtype=np.long) # (1, 32)
        pairs_output_caption_ids = np.zeros((k, self.max_words), dtype=np.long) # (1, 32)
        pairs_decoder_mask = np.zeros((k, self.max_words), dtype=np.long) # (1, 32)

        for i in range(k):
            ind = r_ind[i]
            words, start_, end_ = self._get_single_transcript(data_dict, ind, with_long_context=self.with_long_context) # ['it', 'turns', 'right', 'into', 'this', 'nice', 'thick', 'mixture', 'and', 'like', 'i', 'said', 'i', 'prefer', 'the', 'mixture', 'and', 'like', 'i', 'said', 'i', 'prefer', 'the', 'thinner', 'one', 'and', 'i', "'", 'll', 'just', 'have', 'less', 'thinner', 'one', 'and', 'i', "'", 'll', 'just', 'have', 'less', 'sweet', 'potatoes', 'and', 'it', 'turns', 'to', 'a', 'nice', 'sweet', 'potatoes', 'and', 'it', 'turns', 'to', 'a', 'nice', 'dinner', 'milk', '##sha', '##ke', 'but', 'it', "'", 's', 'absolutely']
            caption_words = words.copy() # (66)
            starts[i], ends[i] = start_, end_ # (122.74, 130.22)
            ...
            if only_sim is False:
                ...
                if len(caption_words) > total_length_with_CLS: # 66, 31 (max-1)
                    caption_words = caption_words[:total_length_with_CLS]
                input_caption_words = ["[CLS]"] + caption_words # (32)
                output_caption_words = caption_words + ["[SEP]"] # (32)

                masked_tokens, token_labels = self._mask_tokens(words) # ['[CLS]', '[MASK]', 'turns', 'right' ... ], [-1, 2009, -1, -1, -1, ... ]
                masked_token_ids = self.tokenizer.convert_tokens_to_ids(masked_tokens) # [101, 103, 4332, 2157, 2046 ...]
                ...
                pairs_input_caption_ids[i] = np.array(input_caption_ids) # (32)
                pairs_output_caption_ids[i] = np.array(output_caption_ids) # (32)
