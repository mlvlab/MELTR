...
class CrossEmbeddings(nn.Module):
    ...
    def forward(self, concat_embeddings, concat_type=None):
        ...
        token_type_embeddings = self.token_type_embeddings(concat_type) # (2, 768) * (60,96) [0, 1]
        position_embeddings = self.position_embeddings(position_ids) # (1024, 768) * (60,96) [0,1,2,... 95]
...
class CrossSelfAttention(nn.Module):
    ...
    def transpose_for_scores(self, x):
        new_x_shape = x.size()[:-1] + (self.num_attention_heads, self.attention_head_size) # (4(B), 96(MW+MF), 12(HN), 64(HS))
        x = x.view(*new_x_shape)
        return x.permute(0, 2, 1, 3) # (4(B), 12(HN), 96(MW+MF), 64(HS))

    def forward(self, hidden_states, attention_mask):
        mixed_query_layer = self.query(hidden_states) # (12(B), 96(MW+MF), 768(D))
        mixed_key_layer = self.key(hidden_states) # (12(B), 96(MW+MF), 768(D))
        mixed_value_layer = self.value(hidden_states) # (12(B), 96(MW+MF), 768(D))

        query_layer = self.transpose_for_scores(mixed_query_layer) # (12(B), 12(HN), 96(MW+MF), 64(HS))
        key_layer = self.transpose_for_scores(mixed_key_layer) # (12(B), 12(HN), 96(MW+MF), 64(HS))
        value_layer = self.transpose_for_scores(mixed_value_layer) # (12(B), 12(HN), 96(MW+MF), 64(HS))

        # Take the dot product between "query" and "key" to get the raw attention scores.
        attention_scores = torch.matmul(query_layer, key_layer.transpose(-1, -2)) # (12(B), 12(HN), 96(MW+MF), 64(HS)) * (12(B, 12(HN), 64(HS), 96(MW+MF)) : (12(B), 12(HN), 96(MW+MF), 96(MW+MF))
        attention_scores = attention_scores / math.sqrt(self.attention_head_size)
        # Apply the attention mask is (precomputed for all layers in CrossModel forward() function)
        attention_scores = attention_scores + attention_mask # (12(B), 12(HN), 96(MW+MF), 96(MW+MF))

        # Normalize the attention scores to probabilities.
        attention_probs = nn.Softmax(dim=-1)(attention_scores) # (12(B), 12(HN), 96(MW+MF), 96(MW+MF))

        # This is actually dropping out entire tokens to attend to, which might
        # seem a bit unusual, but is taken from the original Transformer paper.
        attention_probs = self.dropout(attention_probs)

        context_layer = torch.matmul(attention_probs, value_layer) # (12(B), 12(HN), 96(MW+MF), 96(MW+MF)) * (12(B), 12(HN), 96(MW+MF), 64(HS))
        context_layer = context_layer.permute(0, 2, 1, 3).contiguous() # (12(B), 12(HN), 96(MW+MF), 64(HS)) → (12(B), 96(MW+MF), 12(HN), 64(HS))
        new_context_layer_shape = context_layer.size()[:-2] + (self.all_head_size,) # (12(B), 96(MW+MF), 768(D))
        context_layer = context_layer.view(*new_context_layer_shape) # (12(B), 96(MW+MF), 768(D))
...
class CrossAttention(nn.Module):
    ...
    def forward(self, input_tensor, attention_mask):
        self_output = self.self(input_tensor, attention_mask) # (12(B), 96(MW+MF), 768(D))
        attention_output = self.output(self_output, input_tensor) # (12(B), 96(MW+MF), 768(D))
...
class CrossLayer(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask):
        attention_output = self.attention(hidden_states, attention_mask) # (12(B), 96(MW+MF), 768(D))
        intermediate_output = self.intermediate(attention_output) # (12(B), 96(MW+MF), 3072(D))
        layer_output = self.output(intermediate_output, attention_output) # (12(B), 96(MW+MF), 768(D))
        return layer_output
...
class CrossEncoder(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask, output_all_encoded_layers=True):
        ...
        if not output_all_encoded_layers:
            all_encoder_layers.append(hidden_states)
        return all_encoder_layers # 2 * (5(ss)*12(B), MW(48) + MF(48), D(768))
...
class CrossPooler(nn.Module):
    ...
    def forward(self, hidden_states): # (60, 96, 768)
        # We "pool" the model by simply taking the hidden state corresponding
        # to the first token.
        first_token_tensor = hidden_states[:, 0] # (60, 768)
        pooled_output = self.dense(first_token_tensor) # (60, 768)
        pooled_output = self.activation(pooled_output) # (60, 768)
...
class CrossModel(PreTrainedModel):
    ...
    def forward(self, concat_input, concat_type=None, attention_mask=None, output_all_encoded_layers=True):
        ...
        extended_attention_mask = attention_mask.unsqueeze(1).unsqueeze(2) # (60, 1, 1, 96)
        ...
        embedding_output = self.embeddings(concat_input, concat_type) # (5 * 12(ss * B), 48 + 48(MW + MF), 768)
        encoded_layers = self.encoder(embedding_output,
                                      extended_attention_mask,
                                      output_all_encoded_layers=output_all_encoded_layers) # 2 * (5(ss)*12(B), 48(MW) + 48(MF), 768(D))
        sequence_output = encoded_layers[-1] # (5(ss)*12(B), 48(MW) + 48(MF), 768(D))
        pooled_output = self.pooler(sequence_output) # (5(ss)*12(B), 768(D))
        if not output_all_encoded_layers:
            encoded_layers = encoded_layers[-1]
        return encoded_layers, pooled_output # 2 * (5(ss)*12(B), 48 + 48(MW + MF), 768(D)), (5(ss)*12(B), 768(D))