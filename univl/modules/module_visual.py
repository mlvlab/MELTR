...
# class MeltrConfig(PretrainedConfig):
#     """Configuration class to store the configuration of a `VisualModel`.
#     """
#     pretrained_model_archive_map = PRETRAINED_MODEL_ARCHIVE_MAP
#     config_name = CONFIG_NAME
...
class VisualLMPredictionHead(nn.Module):
    ...
    def forward(self, hidden_states):
        hidden_states = self.transform(hidden_states) # (12(B), 48(MF), 768(D))
        hidden_states = hidden_states.matmul(self.weight) + self.bias
        return hidden_states


class VisualOnlyMFMHead(nn.Module):
    def __init__(self, config, visual_model_embedding_weights):
        super(VisualOnlyMFMHead, self).__init__()
...
class VisualModel(PreTrainedModel):
    ...
    def forward(self, video, attention_mask=None, output_all_encoded_layers=True):
        ...
        extended_attention_mask = attention_mask.unsqueeze(1).unsqueeze(2) # (B, 1, 1, 48)
        ...
        extended_attention_mask = (1.0 - extended_attention_mask) * -10000.0 # (B, 1, 1, 48)

        embedding_output = self.embeddings(video) # (B, 48, 768)
        encoded_layers = self.encoder(embedding_output,
                                      extended_attention_mask,
                                      output_all_encoded_layers=output_all_encoded_layers) # L * (B, MAX_F, D)
        sequence_output = encoded_layers[-1] # (B, MAX_F, D)
        pooled_output = self.pooler(sequence_output) # (B, D)
        if not output_all_encoded_layers:
            encoded_layers = encoded_layers[-1]
        return encoded_layers, pooled_output # L * (B, MAX_F, D), (B, D)
