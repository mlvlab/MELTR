...
class DecoderModel(PreTrainedModel):
    ...
    def forward(self, input_ids, encoder_outs=None, answer_mask=None, encoder_mask=None):
        ...
        """
        embedding_output = self.embeddings(input_ids) # (12(B), 48(MW), 768(D))

        extended_encoder_mask = encoder_mask.unsqueeze(1).unsqueeze(2)   # (12(B), 1, 1, 48+48(MW+MF)
        extended_encoder_mask = extended_encoder_mask.to(dtype=self.dtype) # fp16 compatibility
        extended_encoder_mask = (1.0 - extended_encoder_mask) * -10000.0 # (12, 1, 1, 48+48(MW+MF))

        extended_answer_mask = answer_mask.unsqueeze(1).unsqueeze(2) # (12(B), 1, 1, 48+48(MW+MF))
        extended_answer_mask = extended_answer_mask.to(dtype=self.dtype)

        sz_b, len_s, _ = embedding_output.size() # (12(B), 48(MW), 768(D))
        subsequent_mask = torch.triu(torch.ones((len_s, len_s), device=embedding_output.device, dtype=embedding_output.dtype), diagonal=1) # (48, 48)
        self_attn_mask = subsequent_mask.unsqueeze(0).expand(sz_b, -1, -1).unsqueeze(1)  # (12(B), 1, 48, 48)
        slf_attn_mask = ((1.0 - extended_answer_mask) + self_attn_mask).gt(0).to(dtype=self.dtype) # (12(B), 1, 48(MW), 48(MF))
        self_attn_mask = slf_attn_mask * -10000.0 # (12(B), 1, 48(MW), 48(MF))

        decoded_layers, dec_att_scores = self.decoder(embedding_output, # (12, 48, 768)
                                      encoder_outs, # (12(B), 48+48(MW + MF), 768(D))
                                      self_attn_mask, # (12(B), 1, 48(MW), 48(MF))
                                      extended_encoder_mask, # (12(B), 1, 1, 48+48(MW + MF))
                                      )
                # decoded_layers : (12, 48, 768)
                # dec_att_scores : (12, 12, 48, 96)
        sequence_output = decoded_layers[-1] # (12, 48, 768)
        cls_scores = self.classifier(sequence_output)

        return cls_scores
# self.decoder(embedding_output, encoder_outs, self_attn_mask, extended_encoder_mask, )