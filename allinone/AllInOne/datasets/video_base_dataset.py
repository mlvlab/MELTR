...
class BaseDataset(torch.utils.data.Dataset):
    ...
    def get_raw_video(self, sample):
        ...
        # imgs, idxs, vlen = read_frames_cv2(abs_fp, self.num_frames, mode=self.split)
        if imgs is None:
...
def read_frames_decord(video_path, num_frames, mode='train', fix_start=None):
    ...
    if mode in ['train', 'val']:
        ...
        sample = 'uniform'
    video_reader = decord.VideoReader(video_path, width=512, height=512, num_threads=4, ctx=cpu(0))
    # video_reader = decord.VideoReader(video_path, width=256, height=256, num_threads=2, ctx=cpu(0))
    decord.bridge.set_bridge('torch')
    vlen = len(video_reader)
    frame_idxs = sample_frames(num_frames, vlen, sample=sample, fix_start=fix_start)
    frames = video_reader.get_batch(frame_idxs).to(torch.uint8)
...
def read_frames_from_img_dir(video_path, num_frames, mode='train', fix_start=None, suffix='.jpg'):
    if mode in ['train', 'val']:
        ...
        sample = 'uniform'
    video_path = video_path.replace('videos', 'frames')[:-4]
    vlen = len(os.listdir(video_path))
    frame_idxs = sample_frames(num_frames, vlen, sample=sample, fix_start=fix_start)
    frames = []
    for idx in frame_idxs:
        frame = cv2.imread(f'{video_path}/{idx:06d}{suffix}')
        frame = torch.from_numpy(frame).byte()
        frame = frame.permute(2, 0, 1)
        frames.append(frame)
...
def fast_decode(video_path, num_frames, mode='train', fix_start=None, fps=30):
    ...
    if video.shape[1] < num_frames:
        zeros = torch.zeros((3, num_frames - video.shape[1], size, size), dtype=torch.uint8)
        video = torch.cat((video, zeros), axis=1)
    return video[:, :num_frames], _, _
